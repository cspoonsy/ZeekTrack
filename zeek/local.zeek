...
##! talking to exactly one slave (controller-modbus). MultiSlaveSweep fires
##! when a master fans out to multiple slave unit IDs in a short window —
##! that pattern never occurs in this topology. If a future exercise adds a
##! second outstation, revisit this comment.

# Deployment-specific redefs: authorized_masters and authorized_publishers.
# Generated at container start by docker-entrypoint.sh from MODBUS_MASTER_IP
# and MQTT_PUBLISHER_IP env vars. Keeps LAN IPs out of the baked image so the
# same image works on Docker bridge (172.19.x) and physical Pis (192.168.x).
@load ./deployment
